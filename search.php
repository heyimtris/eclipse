...
<path d="M7 13L1 7M1 7L7 1M1 7H13C14.5913 7 16.1174 7.63214 17.2426 8.75736C18.3679 9.88258 19 11.4087 19 13C19 14.5913 18.3679 16.1174 17.2426 17.2426C16.1174 18.3679 14.5913 19 13 19H10" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                ...
                if (!empty($_GET['text'])) {
                    ...
                    $sql = "SELECT * FROM messages WHERE content LIKE '%$searchText%' AND sender IN (SELECT id FROM users WHERE recipient = '".$_SESSION["id"]."' OR sender = '".$_SESSION["id"]."') ORDER BY timestamp DESC";
                    $result = $conn->query($sql);
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            if ($row['sender'] == $_SESSION["id"]) {
                                ...
                                echo "<div class='search-result' onclick='window.location.assign(`messages.php?user=".$recipientRow['id']."#".$row['id']."`)'>".$senderRow['username'].": ".$row['content']." (".$timestamp.")</div>";
                            } else {
                                echo "<div class='search-result' onclick='window.location.assign(`messages.php?user=".$senderRow['id']."#".$row['id']."`)'>".$senderRow['username'].": ".$row['content']." (".$timestamp.")</div>";
                            }
                        }
                    } else {
  ...
  crossorigin="anonymous"></script>
        <script>
            ...
            function changeStatus() {
                ...
                $.post('/changeStatus.php', { username: "<?php echo $_SESSION["username"];?>", status }, function(result) { 
                    alert(result);
                });
            }
            ...
            function changeCustomStatus() {
                ...
                $.post('/changeCustomStatus.php', { username: "<?php echo $_SESSION["username"];?>", customStatus }, function(result) { 
                    alert(result);
                });
            }
